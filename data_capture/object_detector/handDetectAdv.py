...
class HandDetect():
  ...
  def video(self, videoPath, framesPath):
    if not os.path.exists(framesPath):
            os.mkdir(framesPath)
    # For video path input:
    cap = cv2.VideoCapture(videoPath)  # pass 0 to capture directly from webcam
    with mp_hands.Hands(
        model_complexity=0,
...
if __name__ == "__main__":
    ...
    Detector = HandDetect()
    Detector.video(videoPath, framesPath)